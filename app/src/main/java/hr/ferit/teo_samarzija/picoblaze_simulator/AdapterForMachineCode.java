...
public class AdapterForMachineCode extends RecyclerView.Adapter<AdapterForMachineCode.ViewHolder> {
    @NonNull
    @Override
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.machine_code_directive_representation, parent, false);
...
int index=-1;
    ...
    StringBuilder machineCodeString= new StringBuilder(Integer.toHexString(assembledProgram.instructions[index]));
    while (machineCodeString.length()<5) machineCodeString.insert(0, "0");
    StringBuilder addressString= new StringBuilder(Integer.toHexString(index));
    while (addressString.length()<3) addressString.insert(0, "0");
    holder.getAddressView().setText(addressString.toString());
    holder.getMachineCodeView().setText(machineCodeString.toString());
    holder.getLineNumberView().setText(String.format(Locale.US,"%d",assembledProgram.lineNumbers[index]));
    }
    ...
    public int getItemCount() {
        ...
        AssembledProgram assembledProgram=AssembledProgram.getInstance();
        for (int directive:
             assembledProgram.instructions) {
            if (directive!=0) counter++;
        }
        return counter;
    }
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public TextView getMachineCodeView() {
